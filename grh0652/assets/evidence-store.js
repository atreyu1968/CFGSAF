...
async function call(path,opt={}){if(!API)return null;try{const r=await fetch(API+path,{headers:{'Content-Type':'application/json',...(opt.headers||{})},...opt});const data=await r.json().catch(()=>({}));if(!r.ok){const e=new Error(data.detail||('HTTP '+r.status));e.status=r.status;throw e}return data}catch(e){console.warn('Evidence API:',e.message);return {error:e.message,status:e.status||0}}}
