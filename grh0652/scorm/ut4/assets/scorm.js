...
async function loadStudentFeedback(){
 const box=document.getElementById('studentFeedback');if(!box)return;const ev=evidence();if(!ev||!ev.api||!ev.feedback){box.innerHTML='<p>El feedback requiere conexión con el servidor.</p>';return}
 const rows=await ev.feedback();if(!rows||rows.error){box.innerHTML='<p>No se pudieron cargar las correcciones.</p>';return}
 if(!rows.length){box.innerHTML='<p>Todavía no hay correcciones validadas.</p>';return}
 box.innerHTML=rows.map(r=>'<article class="exercise done"><div class="type">CE '+safe(r.ce||'')+' · '+safe(r.item_id||'')+'</div><h3>'+Math.round(Number(r.score||0))+' / 100</h3><p>'+safe(r.feedback||'')+'</p>'+(r.breakdown&&r.breakdown.length?'<div class="ce-table">'+r.breakdown.map(x=>'<div><b>'+safe(x.name)+' · '+Number(x.weight||0)+'%</b><span>'+Math.round(Number(x.score||0))+'/100 · '+safe(x.feedback||'')+'</span></div>').join('')+'</div>':'')+'</article>').join('');
}
